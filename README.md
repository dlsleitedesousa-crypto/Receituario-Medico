...
No receituário, **APAC** abre os campos de procedimento, código SIGTAP automático, quantidade, CID-10, diagnóstico e observações. **Gerar PDF da APAC** preenche diretamente o modelo `assets/apac-modelo.pdf` fornecido pelo usuário, mantendo a página e o desenho originais. Nome e data de nascimento do paciente, estabelecimento, procedimento, CID, diagnóstico, observações e dados disponíveis do profissional são inseridos nos campos correspondentes. Os campos de autorização ficam em branco. O arquivo abre em outra aba para impressão ou salvamento. Publique também `apac.js`, `apac-template-pdf.js`, `apac.css`, o modelo PDF e `vendor/pdf-lib-1.17.1.min.js`.
